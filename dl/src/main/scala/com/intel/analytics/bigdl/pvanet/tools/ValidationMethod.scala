...
trait ValidationMethod[T] {
  def apply(output: Table, target: Table): ValidationResult
...
class Top1Accuracy[T] extends ValidationMethod[T] {
  override def apply(output: Table, target: Table): ValidationResult = {
    var correct = 0
    var count = 0
...
class Top5Accuracy[T] extends ValidationMethod[T] {
  override def apply(output: Table, target: Table): AccuracyResult = {
    var correct = 0
    var count = 0
//    if (output.dim() == 2) {
